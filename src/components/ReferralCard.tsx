...
export const ReferralCard = () => {
  ...
  useEffect(() => {
    const fetchReferralData = async () => {
      ...
      const { data: profile } = await supabase
        .from("profiles")
        .select("referral_code, bonus_daily_files, bonus_expires_at")
  ...
  const hasActiveBonus = referralData.bonus_expires_at && 
    new Date(referralData.bonus_expires_at) > new Date() && 
    referralData.bonus_daily_files > 0;

  const bonusExpiryDate = referralData.bonus_expires_at 
    ? new Date(referralData.bonus_expires_at).toLocaleDateString("en-US", {
        month: "short",
        day: "numeric",
        year: "numeric",
      })
  ...
  return (
    <Card className="shadow-card bg-card border-border/50">
      <CardHeader>
        ...
        <CardDescription>
          Invite friends and earn +10 bonus file protections per day for 30 days when they upgrade!
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {/* Bonus Expiry */}
        {hasActiveBonus && bonusExpiryDate && (
