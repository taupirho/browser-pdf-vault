...
CREATE POLICY "No one can select DSAR requests"
ON public.dsar_requests
FOR SELECT
USING (false);

CREATE POLICY "No one can update DSAR requests"
ON public.dsar_requests
FOR UPDATE
USING (false);

CREATE POLICY "No one can delete DSAR requests"
ON public.dsar_requests
FOR DELETE
USING (false);
...
CREATE POLICY "No one can select audit logs"
ON public.audit_logs
FOR SELECT
USING (false);
